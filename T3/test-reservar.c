...
static pthread_mutex_t stat_mon= PTHREAD_MUTEX_INITIALIZER;
...
int vehiculoFun(Vehiculo *v, int iter, int k, int tiempo) {
  ...
  while (iter--) {
    ...
    if (verbose)
      printf("%d: %s reserva %d estacionamiento desde %d: %s\n",
              tiempoActual(), v->nom, k, e, status);
    pthread_mutex_unlock(&stat_mon);
    if (tiempo>0)
      pausa(tiempo);
    pthread_mutex_lock(&stat_mon);
    for (int j= e; j<e+k; j++) {
    ...
    if (verbose)
      printf("%d: %s libera %d estacionamientos desde %d: %s\n",
              tiempoActual(), v->nom, k, e, status);
    pthread_mutex_unlock(&stat_mon);
    liberar(e, k);
  }
